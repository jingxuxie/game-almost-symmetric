...
\]
Both directions are present because all ordered action pairs are included.  A $G$-invariant surrogate is specified by one potential $x_v$ for each orbit, with $\widehat u_i(a)=x_{[i,a]}$.
...
\end{equation}
At the optimum, shortest-path potentials for edge lengths $c_e+\delta_G$ define a nearest invariant surrogate.  Hence $\delta_G$ and a witness cycle can be computed in polynomial time by a maximum-mean-cycle algorithm.
\end{theorem}
\begin{proof}
...
$t\ge-|C|^{-1}\sum_{e\in C}c_e$.  Minimizing $t$ gives \eqref{eq:cycle-characterization}.  At the critical value there are no negative cycles, so shortest-path distances from an added source provide feasible potentials.  Karp's algorithm finds the extremal cycle mean in polynomial time \cite{karp1978cycle}.
\end{proof}
