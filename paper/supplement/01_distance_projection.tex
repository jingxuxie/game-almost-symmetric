...
\]
For a fixed slice $(i,a_{-i})$, the pairwise maximum is the residual range:
\begin{align}
\max_{a_i,b_i}
&|r_i(a_i,a_{-i})-r_i(b_i,a_{-i})|
\nonumber\\
...
$r_i(a_i,a_{-i})$ lie in an interval of width at most $\delta$.  The expectations under $p_i$ and $q_i$ are two convex combinations inside that interval, so their difference has magnitude at most $\delta$.  Averaging over $\sigma_{-i}$ preserves the bound.
\end{proof}

\subsection{Exactness of the Sparse Projection LP}

Let $S$ generate $G$.  The projection program has payoff variables $\widehat u_i(a)$, residual bounds $L_{i,a_{-i}},U_{i,a_{-i}}$, and $t\ge0$:
\begin{align}
\min\quad&t
\label{eq:supp-lp}\\
\text{s.t.}\quad
...
\]
and the payoff variables define a nearest invariant surrogate.
\end{theorem}
\begin{proof}
...
Invariance under each generator implies invariance under every finite product and hence under $G$.  Conversely, every game in $\Fix(G)$ satisfies the generator equalities.  The program therefore minimizes precisely over $\Fix(G)$.
\end{proof}

If $P=n\prod_i|A_i|$ is the number of payoff entries and
$Q=\sum_i\prod_{j\ne i}|A_j|$ the number of slices, the formulation uses $P+2Q+1$ variables and $2P+Q$ strategic inequalities, plus sparse generator equalities.  Additional equalities preserve zero-sum, common-payoff, or other convex linear structure.
